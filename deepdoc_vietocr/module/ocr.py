...
def load_model(model_dir, nm, device_id: int | None = None):
    ...
    if cuda_is_available():
        cuda_provider_options = {
            ...
            "gpu_mem_limit": 2 * 1024 * 1024 * 1024, # Limit gpu memory to 2GB
...
class TextRecognizer:
    def __init__(self, model_dir=None, device_id: int | None = None):
        ...
        # Fix path for macOS/Linux instead of hardcoded windows path
        config['weights'] = os.path.join(os.path.dirname(os.path.abspath(__file__)), '..', 'vietocr', 'weight', 'vgg_seq2seq.pth')
